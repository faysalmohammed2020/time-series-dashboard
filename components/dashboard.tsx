  ...
  const loadData = async (forceRefresh = false) => {
    try {
      ...
      if (useSampleData) {
        ...
        setData(sampleData)
        setColumnTypes(getColumnTypes(sampleData))
        setLoading(false)
        return
      }
      ...
      if (!forceRefresh && isCachedDataFresh()) {
        ...
        if (cachedData && cachedData.length > 0) {
          ...
          setColumnTypes(getColumnTypes(processedData))
          setLoading(false)
          return
        }
      }
      ...
      // Update state
      setData(parsedData)
      setColumnTypes(getColumnTypes(parsedData))
      setLoading(false)
    } catch (err) {
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="charts">
          ...
          <TabsContent value="charts" className="space-y-6">
            ...
            {/* Individual charts for each field */}
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-sm">
                  <p>
                    <span className="font-medium">Data Source:</span> {useSampleData ? "Sample Data" : "CSV File"}
                  </p>
                  <p>
                    <span className="font-medium">Rows:</span> {data.length}
                  </p>
                  <p>
                    <span className="font-medium">Time Column:</span> {timeColumn}
                  </p>
                  <p>
                    <span className="font-medium">Fields Displayed:</span>{" "}
                    {fieldsToChart.length > 0 ? fieldsToChart.join(", ") : "None detected"}
                  </p>
                </div>
              </CardContent>
            </Card>
