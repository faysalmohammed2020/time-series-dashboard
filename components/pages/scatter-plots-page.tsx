...
export default function ScatterPlotsPage() {
  ...
  // Create pairs of fields for scatter plots
  const scatterPairs = []
  for (let i = 0; i < fieldsToChart.length; i++) {
    for (let j = i + 1; j < fieldsToChart.length; j++) {
      // Only add some interesting combinations to avoid too many charts
      if (
        // Temperature vs humidity
        (fieldsToChart[i].toLowerCase().includes("temp") && fieldsToChart[j].toLowerCase().includes("humid")) ||
        (fieldsToChart[j].toLowerCase().includes("temp") && fieldsToChart[i].toLowerCase().includes("humid")) ||
        // Wind speed vs direction
        (fieldsToChart[i].toLowerCase().includes("windspeed") &&
          fieldsToChart[j].toLowerCase().includes("direction")) ||
        (fieldsToChart[j].toLowerCase().includes("windspeed") &&
          fieldsToChart[i].toLowerCase().includes("direction")) ||
        // Pressure vs temperature
        (fieldsToChart[i].toLowerCase().includes("pressure") && fieldsToChart[j].toLowerCase().includes("temp")) ||
        (fieldsToChart[j].toLowerCase().includes("pressure") && fieldsToChart[i].toLowerCase().includes("temp")) ||
        // Solar vs temperature
        (fieldsToChart[i].toLowerCase().includes("solar") && fieldsToChart[j].toLowerCase().includes("temp")) ||
        (fieldsToChart[j].toLowerCase().includes("solar") && fieldsToChart[i].toLowerCase().includes("temp")) ||
        // Just add some pairs if we don't have enough
        scatterPairs.length < 4
      ) {
        scatterPairs.push([fieldsToChart[i], fieldsToChart[j]])
        // Limit to 8 scatter plots
        if (scatterPairs.length >= 8) break
      }
    }
    if (scatterPairs.length >= 8) break
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold">Scatter Plots</h1>
          <div className="flex gap-2">
        ...
        {/* Scatter plots for pairs of fields */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {scatterPairs.map(([xField, yField], index) => (
            <ScatterPlot
              key={`${xField}-${yField}`}
              data={data}
              xColumn={xField}
              yColumn={yField}
              title={`${xField.charAt(0).toUpperCase() + xField.slice(1).replace(/_/g, " ")} vs ${
                yField.charAt(0).toUpperCase() + yField.slice(1).replace(/_/g, " ")
              }`}
            />
          ))}
        </div>
